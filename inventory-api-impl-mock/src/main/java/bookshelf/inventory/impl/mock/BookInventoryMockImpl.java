...
public class BookInventoryMockImpl implements BookInventory {
    ...
    public MutableBook loadBookForEdit(String isbn) throws BookNotFoundException {
        MutableBook book = booksByIsbn.get(isbn);
        if (book == null) {
    ...
    public Set<String> searchBooks(Map<SearchCriteria, String> criteria) {
        ...
        books.addAll(booksByIsbn.values());
            ...
            while (it.hasNext()) {
                ...
                switch (criterion.getKey()) {
                    ...
                    case CATEGORY_LIKE:
                        ...
                        break;
                    case TITLE_LIKE:
                        if (!checkStringMatch(book.getTile(), criterion.getValue())) {
                            it.remove();
                            continue;
                        }
                        break;
                    case RATING_GT:
                        if (!checkIntegerMatch(book.getRating(), criterion.getValue())) {
                            it.remove();
                            continue;
                        }
                        break;
    ...
    private boolean checkIntegerMatch(int attr, String critVal) {
        int critValInt;
        try {
